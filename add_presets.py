...
class Inference:
    def __init__(
        ...
        self.normalizer = Normalizer('http://192.168.2.233:9009/api/v1/normalize')
    ...
    def _load_flow_decoder(
        ...
        flow_decoder.model = torch.jit.load(
            "/mnt/shared_data/users/artyom/projects/castle-ai-model-convert-factory/converted_models/mel_spec_flow_decoder/v2/mel_spec_flow_decoder_v2_1.pt")
        flow_decoder.eval()
    ...
    def prepare_prompt(
        ...
        if transcribe:
            ...
            text = self.normalizer.normalize(text).normalized_text
...
if __name__ == "__main__":
    ...
    # voices_metadata = pd.read_csv('/mnt/shared_data/users/artyom/projects/add_voice_presets/voices_prompts.csv')
    voices_metadata = pd.read_csv('/mnt/shared_data/users/sveta/projects/eval_repo/castle-ai-speech-evaluation/pitch_shifted_meta.csv')
    force_insert = True

    if not force_insert:
        logging.warning("Force insert disabled, voice will be able to be updated")
        verification = input("Are you sure to update existing voice presets ? ")
    else:
        logging.warning("Force insert disabled, voice preset will not be able to update")
        verification = input("Are you sure to update existing voice presets ? ")

    if verification != "Yes":
        raise RuntimeError(f"Verification did not pass: {verification}")
    ...
    for _, row in voices_metadata.iterrows():
        ...
        speaker_id, task_id, gender, speaker_name = row['Speaker ID'], row['Task ID'], row['Gender'], row['Speaker Name']
        prompt_audio_path = row['prompt_audio_peocessed_pitch_path']
        prompt_text = row['prompt_text_processed']
        ...
        pbar.update(1)
    print(f"Saved {voices_metadata.shape[0]} voice presets.")
